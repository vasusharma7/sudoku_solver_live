...
def box_extraction(root, img_for_box_extraction_path, cropped_dir_path):
    ...
    while i < (h - h % 9):
        j = 0
        while j < (w-w % 9):
            new_img = img[i:i+h//9, j:j+w//9]
            cv2.imwrite(f'./output/temp{idx}.png', new_img)
            idx += 1
            j += w//9
        i += h//9
    print(x, y, w, h)
...
box_extraction(True, "./images.png", "./output/", )
